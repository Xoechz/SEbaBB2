...
TYPE 
  ...
  WordString = STRING[maxWordLen]; (*to save memory, longer words are stripped*)

PROCEDURE OpenFile(fileName: STRING; c: Conversion);
PROCEDURE ReadWord(VAR w: WordString); (*w = '' on end of file*)
PROCEDURE CloseFile;
...
PROCEDURE ConvertToLower(VAR w: WordString);
VAR 
  i: INTEGER;
BEGIN
...
PROCEDURE ConvertToUpper(VAR w: WordString);
VAR 
  i: INTEGER;
BEGIN
  FOR i := 1 TO Length(w) DO
...
END; (*OpenFile*)
  ...
  (*-------------------------------------------------------------*)
PROCEDURE ReadWord(VAR w: WordString);
VAR 
  i: INTEGER;
BEGIN
  w := '';
  WHILE (ch <> EF) AND NOT (ch IN characters) DO
    BEGIN
      NextChar;
    END; (*WHILE*)
  IF ch = EF THEN
